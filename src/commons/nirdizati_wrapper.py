...
def predict(path_to_predictors: str, path_to_event_log: str,
            save_loc: str, q: mp.Queue):
    try:
        # Set path
        ...
        for predictor in predictor_iter:
            predictor_result, predictor_aggregate = predict_multi(
                path_to_event_log, predictor.path)
            ...
            if init_results:
                ...
                combined_results = pd.merge(
                    combined_results, predictor_result, on="Case ID")
        ...
        combined_results.to_csv(
            f"{save_loc}-results.csv", mode="a", sep=",", index=False)
...
def train(path_to_config: str, path_to_schema: str,
          path_to_event_log: str, save_loc: str, q: mp.Queue):
    try:
        # Set path
        sys.path.insert(0, os.path.join(
            "commons", "nirdizati-training-backend", "core"))

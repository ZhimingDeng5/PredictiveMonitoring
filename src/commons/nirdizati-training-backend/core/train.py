...
def train(train_file, config_file, params_file, save_loc):
    ...
    pickle_file = f"{save_loc}-predictor.pkl"
    ...
    outfile = f"{save_loc}-validation.csv"
    ...
    detailed_results_file = f"{save_loc}-detailed.csv"
    ...
    with open(str(outfile), 'w') as fout:
        ...
        for bucket in set(bucket_assignments_train):
            ...
            importances.head(20).to_csv(f"{save_loc}-feat-importance.csv")

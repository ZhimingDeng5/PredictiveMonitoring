    ...
    try:
        for index, Pipeline in data.items():
            ...
            if not type_check(Pipeline, ["Pipeline"]):
                return response(False, str(type(Pipeline)) +
                                " is not Pipeline")
            for step in Pipeline.steps:
                ...
                if step_name == "encoder":
                    if not type_check(step[1], ["FeatureUnion"]):
                        return response(False, str(
                            type(step[1])) + " is not FeatureUnion")
                    for transformer_item in step[1].transformer_list:
                        transformer_name = transformer_item[0]
                        if transformer_name not in ["static", "agg"]:
                            return response(
                                False, transformer_name + " is not static or agg")
                        if transformer_name == "static":
                            if not type_check(transformer_item[1], [
                                              "StaticTransformer"]):
                                return response(
                                    False, str(
                                        type(
                                            transformer_item[1])) + " is not StaticTransformer")
                            ...
                            if not type_check(transformer_item[1], [
                                ...
                                return response(
                                    False, str(
                                        type(
                                            transformer_item[1])) + " is not AggregateTransformer")
                elif step_name == "cls":
                    if not type_check(step[1], ["ClassifierWrapper"]):
                        return response(False, str(
                            type(step[1])) + " is not ClassifierWrapper")
        ...
        for key, value in items:
            ...
            if str(value).find('[') != -1:
                for name in list(value):
                    if str(key).lower().find('num') != - \
                            1 or str(key).lower().find('ignore') != -1:
                        if types[name] is None or (
                                types[name] != "int64" and types[name] != "float64"):
                            return response(
                                False, '\"' + str(name) + '\" should be a number')
                    elif str(key).lower().find('timestamp') != -1:
                        if types[name] is None or not check_timestamp(
                                cf[name]):
                            return response(
                                False, '\"' + str(name) + '\" should be a timestamp')
                    else:
                        if types[name] is None or \
                                (types[name] != "object" and types[name] != "int64" and
                                 types[name] != "float64" and types[name] != "bool"):
                            return response(
                                False, '\"' + str(name) + '\" should be a object')
            else:
                name = value
                if str(key).lower().find('num') != -1 or str(key).lower().find(
                    ...
                    if types[name] is None or not check_timestamp(cf[name]):
                        return response(False, '\"' + str(name) +
...
def validate_csv_in_path(csv_path: str, schema_path: str):
    try:
        ...
        cf = pd.read_csv(csv_path, index_col=False,
                         low_memory=False, delimiter=de, nrows=200)
        types = cf.dtypes
...
def validate_config(config_path: str, schema_path: str):
    try:
        ...
        if len(config_json) > 3:
            return response(
                False, "config file should only have two or three attributes")
        schema = open(schema_path).read()
        ...
        if target_key != "remtime" and \
            ...
            return response(False, target_key +
                            " is not a parameter of config json")
        ...
        if bucketing_type not in ["zero", "cluster", "state", "prefix"]:
            return response(False, bucketing_type +
                            " is not a parameter of a bucket")
        is_cluster = False
        ...
        if encoding_type not in ["agg", "laststate", "index", "combined"]:
            return response(False, encoding_type +
                            " is not a parameter of an encoding")
        ...
        if learner_type == "rf":
            if is_cluster and len(l_v) != 3:
                return response(False, learner_type +
                                " should only have three attributes")
            if not is_cluster and len(l_v) != 2:
                return response(False, learner_type +
                                " should only have two attributes")
            for p, p_v in l_v.items():
                if p not in ["n_clusters", "n_estimators", "max_features"]:
                    return response(
                        False, p + " is not a parameter of " + learner_type)
                if "float" not in str(
                        type(p_v)) and "int" not in str(type(p_v)):
                    return response(False, p + " should be a number")
        elif learner_type == "gbm":
            if is_cluster and len(l_v) != 4:
                return response(False, learner_type +
                                " should only have four attributes")
            if not is_cluster and len(l_v) != 3:
                return response(False, learner_type +
                                " should only have three attributes")
            for p, p_v in l_v.items():
                if p not in ["n_clusters", "n_estimators",
            ...
            if is_cluster and len(l_v) != 6:
                return response(False, learner_type +
                                " should only have six attributes")
            if not is_cluster and len(l_v) != 5:
                return response(False, learner_type +
                                " should only have five attributes")
            for p, p_v in l_v.items():
                if p not in ["n_clusters", "n_estimators", "max_depth",

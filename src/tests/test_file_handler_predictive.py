...
pickle_file = [UploadFile('test-label-predictor.pkl',
                          open(pickle_address, 'rb'))]
...
def test_savePredictEventlog():
    ...
    expect = os.path.join('../', 'predict_files',
                          new_taskID, 'test-event-log.csv')
    actual = fh.savePredictEventlog(new_taskID, csv_file, '../')
    assert expect == actual

    expect = os.path.join('../', 'predict_files', taskID,
                          'test-event-log.parquet')
...
def test_loadPredictResult():
    ...
    file = UploadFile(taskID + '-results.csv',
                      open('test_files/test-event-log.csv', 'rb'))

...
class WorkerNode:
    def __init__(self):
        ...
        self.worker_thread = WorkerConsumerThread(
            self.cancellations, Service.PREDICTION)
    ...
    def start(self):
        ...
        def cancel_callback(ch, method, properties, body):
            ...
            if was_cancelled and self.cancellations.hasCancel(taskID):
                ...
                print(
                    f"Another node cancelled task with ID {taskID}. Removed it from the cancel set.")

            # the node is currently working on this task. Tell it to stop and
            # have it perform cleanup.
            elif taskID == self.cancellations.getCurrentTask():
                self.worker_thread.cancelTask()

            # the task is in the queue or at another worker node. Add its id to
            # cancel set
            else:
                self.cancellations.addCancel(taskID)
                print(
                    f"Received request to cancel task with ID {taskID}. Added its ID to cancel set.")
        ...
        con, chn = subscribeToQueue(
            cancel_set_request_callback, 'cancel_set_request_p', con, chn)

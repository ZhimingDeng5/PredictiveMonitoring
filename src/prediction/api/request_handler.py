...
def create_dashboard(predictors: List[UploadFile] = File(...),
    ...
    for pfile in predictors:
        ...
        if not res['isSuccess']:
            ...
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="fail to validate predictor: " +
                pfile.filename +
                " [" +
                res['msg'] +
                "]")
    ...
    new_task: Task = Task(
        ...
        config_path="",
        schema_path=fh.loadPredictSchemaAddress(
            uuid,
            schema.filename),
...
def download_result(taskID: str):
    ...
    if tasks.hasTask(taskUUID):
        ...
        return FileResponse(
            fh.loadPredictResult(taskID),
            background=BackgroundTask(
                fh.removePredictTaskFile,
                uuid=taskID))

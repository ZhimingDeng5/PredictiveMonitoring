...
class PersistenceNode:
    ...
    def start(self):
        ...
        con, chn = subscribeToFanout(
            cancel_callback, 'cancellations_t', 'persistent_cancel_queue_t')
        con, chn = subscribeToQueue(
            set_request_callback, 'cancel_set_request_t', con, chn)
        con, chn = subscribeToQueue(
            task_callback, 'persistent_task_status_t', con, chn)
        chn.start_consuming()
